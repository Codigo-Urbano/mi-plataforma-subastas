...
export async function GET(request: Request) {
  ...
  try {
      ...
      if (pujaGanadora) {
        ...
        if (vendedor && comprador && process.env.RESEND_API_KEY) {
          await resend.emails.send({
            ...
            to: [vendedor.email],
            subject: `Tu subasta "${subasta.titulo}" ha finalizado con exito!`,
            html: `
              <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaeb; border-radius: 10px;">
                ...
                <ul style="color: #555; line-height: 1.8; background: #f9f9f9; padding: 15px 35px; border-radius: 5px;">
                  ...
                  <li><strong>Telefono:</strong> ${comprador.telefono || "No especificado"}</li>
                </ul>
                <p style="color: #555; line-height: 1.5; margin-top: 20px;">Por favor, contactate con el ganador para coordinar el pago y la entrega del producto.</p>
              </div>
            `
          });
            ...
            html: `
              <div style="font-family: Arial, sans-serif; padding: 20px; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaeb; border-radius: 10px;">
                ...
                <ul style="color: #555; line-height: 1.8; background: #f9f9f9; padding: 15px 35px; border-radius: 5px;">
                  ...
                  <li><strong>Telefono:</strong> ${vendedor.telefono || "No especificado"}</li>
                </ul>
                <p style="color: #555; line-height: 1.5; margin-top: 20px;">Por favor, contactate con el vendedor para coordinar el pago y la entrega del producto.</p>
              </div>
            `
          });
    ...
    console.error("Error en Cron Job de Finalizacion:", error);
