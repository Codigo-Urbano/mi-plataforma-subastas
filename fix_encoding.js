...
const fs = require("fs");
const path = "src/app/api/cron/finalizar-subastas/route.ts";

let content = fs.readFileSync(path, "utf-8");
// Reemplazar caracteres corruptos comunes
content = content.replace(//g, "!");
content = content.replace(//g, "é");
content = content.replace(//g, "á");
content = content.replace(//g, "ó");

// Para estar seguros, reescribiremos todo el archivo con UTF-8 limpio
const cleanContent = `import { NextResponse } from "next/server";
...
fs.writeFileSync(path, cleanContent.replace(/\\\\\\\`/g, "\\\`"), "utf8");
console.log("Archivo reescrito con exito");
